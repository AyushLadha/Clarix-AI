...
# --------------- Authentication Gate ---------------
# We will show the login/signup UI if the user is not logged in, and show the main app if they are logged in. 
# This way we can restrict access to the app features to only authenticated users, and also provide a personalized experience by showing the user's name and email when they are logged in.

if not st.session_state.user:
    # Show the login/signup UI
    st.title("✦ Clarix Login")
    ...
    with tab2:
        ...
        signup_password = st.text_input("Password", type = "password", key = "signup_password", help = "Must be at least 6 characters")
        signup_password_confirm = st.text_input("Confirm Password", type = "password", key = "signup_password_confirm")

        if st.button("Create Account", type = "primary", use_container_width = True):
            clean_email = signup_email.strip() if signup_email else ""
            clean_name = signup_name.strip() if signup_name else ""

            if not clean_name or not clean_email or not signup_password:
                ...
                with st.spinner("Creating account..."):
                    success, msg = signup_user(clean_email, clean_name, signup_password)
                if success:
                    st.success(msg)
    ...
    st.stop() # Stop execution here if not logged in, so that the main app code below only runs for authenticated users
...
with st.sidebar:
    ...
    if app_mode == "📈 Data Analysis":
        ...
        rag_collection_name = f"rag_user_{user_id}_docs"
        try:
